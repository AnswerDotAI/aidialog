...
# %% auto #0
__all__ = ['PartType', 'tool_info', 'usage_info', 'think_start', 'think_end', 're_think', 'fence_call_re', 'Part', 'Msg',
           'ToolCall', 'display_list', 'mk_tool_res_msg', 'sys_text', 'part_txt', 'data_url', 'url_mime', 'MediaUrl',
           'mk_content', 'parse_tools', 'strip_tools', 'conv_tools', 'extract_fence_call', 'mk_result_fence',
           'split_fence_msgs', 'fmt2hist', 'ToolResponse', 'StopResponse', 'FullResponse', 'trunc_str', 'mk_tr_details',
           'hist2fmt']
    ...
    for msg_part in msg.content:
        if msg_part.type == PartType.thinking: asst_parts.append(msg_part)
            ...
            for i,p in enumerate(parts):
                ...
                else:          res.append(Msg(role='user', content=[Part(type=PartType.text, text=mk_result_fence(p))]))
...
def _trunc_content(content, mx):
    ...
    return trunc_str(content, mx=mx)
...
def mk_tr_details(tr, mx=2000):
    ...
    args = {k:trunc_str(v, mx=None if mx is None else mx*5) if isinstance(v, str) else v for k,v in tr.data['arguments'].items()}
...
@patch(as_prop=True)
def formatted(self:Part):
    "Display form: text as itself, thinking as 🧠 (or the thought when stamped `showthink`), a pending tool call as an ⏳ row, a result as its `{.tool}` block"
    d = self.data or {}
    if self.type==PartType.thinking: return (self.text or '') if d.get('showthink') else '🧠'
    if self.type==PartType.tool_use: return '' if d.get('server') else f"\n- ⏳ {_tc_summary(self)} ⏳\n"
    if self.type==PartType.tool_result: return mk_tr_details(self)
    return self.text or ''
...
def hist2fmt(msgs:list[Msg], mx=2000, showthink=False)->str:
    ...
    for m in msgs:
        if m.role == 'assistant':
            for p in m.content:
                ...
                elif p.type == PartType.thinking and showthink and p.text:
                    out.append(f'{think_start}\n<details><summary>Thinking</summary>\n\n{p.text.strip()}\n\n</details>\n{think_end}')
                elif p.type == PartType.tool_use:
                    if (p.data or {}).get('server'):
                        out.append(mk_tr_details(Part(type=PartType.tool_result, text='Server tool call executed.', data=p.data), mx=mx).strip())
                    else: tus[p.data['id']] = p
        elif m.role == 'tool':
            for p in m.content:
                if p.type != PartType.tool_result: continue
                tu = tus.pop(p.data['id'], None)
    ...
    out += [p.formatted.strip() for p in tus.values()]
