...
def cell_type(self:Message):
    ...
    return 'markdown' if self.msg_type==snote else 'code' if self.msg_type==sprompt else self.msg_type
...
def select_msgs(self:Dialog, *ids, **kwargs):
    "The messages a non-interactive run executes: `select_cells` mechanics (ids, above/below, the eval cascade) over the notebook projection, code messages only"
    return [m for m in select_cells(self, *ids, **kwargs) if m.msg_type==scode]
...
def execute(self:Dialog,
    ...
    **kwargs
    ...
    res = []
    for m in self.select_msgs(*ids, **kwargs):
