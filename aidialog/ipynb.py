...
# %% auto #0
__all__ = ['att2dict', 'home_atts', 'unhome_atts', 'msgs2ipynb', 'get_ipynb', 'safe_mtime', 'write_ipynb', 'ipynb_cells',
           'dict2att', 'reads_ipynb', 'read_ipynb', 'conv_old_prompts']

# %% ../nbs/02_ipynb.ipynb #d3d0463b
from fastcore.utils import *
from fastcore.xtras import atomic_save
from fastcore.nbio import NbCell,mk_cell,new_nb,dict2nb,read_nb,nb2str,repair_cell,repair_nb
import json
...
def msgs2ipynb(msgs, meta=None, version=2):
    "Notebook object from messages and notebook metadata"
    cells = [m.to_cell(version=version) for m in msgs]
    nb = new_nb(cells=cells, meta=obj2dict(meta))
    home_atts(nb)
...
def from_cell(cls:Message, cell, dlg=None):
    ...
    kwargs = {a: meta.pop(k) for a,k in cls.meta_attrs.items() if k in meta}
